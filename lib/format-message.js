...
module.exports = ({
  ...
  colorize = require('colorette').isColorSupported,
  colorizeMessage = require('pino-pretty').colorizerFactory(colorize).message,
} = {}) => {
  return event => {
    const { from, caller = from, name, msg } = event
    const message = msg ? ` : ${colorizeMessage(msg)}` : ''

    if (!colorize) return `${caller || name}${message}`
